...
- Live Vectorize search must retrieve the broadest provider-supported candidate pool with metadata before reranking to avoid obvious topical misses on common queries such as `web development` and normalized singular/plural terms like `algorithm` and `algorithms`.
