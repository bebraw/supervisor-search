...
import { tokenizeSearchText } from "./parser.ts";
...
export function rankSupervisorMatches(
  ...
  return candidates
    ...
    .filter((result) => result.signals.topicOverlap > 0)
    .sort((left, right) => {
      return right.score - left.score || left.activeThesisCount - right.activeThesisCount || left.name.localeCompare(right.name);
    });
}
